...
void main() {
  group('Compress gzip algorithm', () {
    test('compress and decompress with gzip', () async {
      ...
      final compressed = await funx.Func1<String, String>(
        (data) async => data,
      ).compress()(original);
      expect(compressed, isA<String>());
      expect(compressed.length, lessThan(original.length));

      final decompressed = await funx.Func<String>(
        () async => compressed,
      ).decompress()();
    ...
    test('compressBytes with gzip round trip', () async {
      ...
      final decompressed = await funx.Func<Uint8List>(
        () async => compressed,
      ).decompressBytes()();
  ...
  group('Compress thresholds', () {
    test('returns original when below threshold', () async {
      const short = 'hi';
      final result = await funx.Func1<String, String>(
        (data) async => data,
